...
public class AI : MonoBehaviour {
    ...
    Stack<Move> GenerateMoves(Dictionary<Vector3, GameManager.GameGridCell> gameGrid, bool isWhite) {
        ...
        if (isQueenPlaced) {
            foreach (KeyValuePair<Vector3, GameManager.GameGridCell> gridTile in gameGrid) {
                if (gridTile.Value.isFilled) {
                    Stack<Vector3> movePositions = gameManager.ValidateMoves(gridTile.Value.tile);
    ...
    int ScoreGame(Dictionary<Vector3, GameManager.GameGridCell> gameGrid, bool isAI) {
    ...
    int TilesAroundQueenHeuristic(Dictionary<Vector3, GameManager.GameGridCell> gameGrid, bool isAI) {
        int score = 0;
    ...
    bool IsWinningMove(Dictionary<Vector3, GameManager.GameGridCell> gameGrid, bool isAI) {
    ...
    int GetBoarderTiles(Dictionary<Vector3, GameManager.GameGridCell> gameGrid, GameObject tile) {
        Vector3 newPos;
        int boarderTiles = 0;
    ...
    Dictionary<Vector3, GameManager.GameGridCell> SimulateMove(Dictionary<Vector3, GameManager.GameGridCell> gameGrid, Move move) {
        Dictionary<Vector3, GameManager.GameGridCell> ret = gameGrid;
        ...
        if (isMove) {
            ret[tile.transform.position] = new GameManager.GameGridCell();
            tile.transform.position = pos;

            GameManager.GameGridCell gridCell = new GameManager.GameGridCell(true, tile);
